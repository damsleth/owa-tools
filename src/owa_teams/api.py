...
Both doors return parsed JSON on success and raise typed OwaError subclasses
on failure; the dispatcher owns rendering and exit codes.
...
def graph_collect(base, endpoint, access_token, *, top=None, max_pages=50,
    ...
    truncated = []
    pages = http.paginate(
        url, token=access_token, max_pages=max_pages, retry=retry, debug=debug,
        on_truncate=lambda *_: truncated.append(True),
    )
    for item in pages:
        rows.append(item)
...
def chatsvc_messages(base, conversation_id, access_token, *, page_size=50, max_pages=20,
    ...
    Returns the concatenated raw `messages` (newest-first across pages),
    raising a typed OwaError on failure. Bounded by `max_pages` so an enormous channel
...
def chatsvc_post(base, conversation_id, body, access_token, debug=False, retry=DEFAULT_RETRY):
    """POST a message to a chatsvc conversation; return parsed JSON or raise OwaError.
    ...
    codes; other failures also propagate as typed errors.
    """
