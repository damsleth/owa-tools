...
def test_do_token_refresh_missing_binary(monkeypatch, capsys):
    _patch_owa_piggy(monkeypatch, fake_run=lambda *a, **k: FakeProc(), available=False)
    assert auth_mod.do_token_refresh({}) is None
    err = capsys.readouterr().err
    assert 'owa-piggy not found' in err
    assert 'damsleth/tap/owa-piggy' in err


def test_do_token_refresh_happy_path_forwards_audience_and_profile(monkeypatch):
    captured = []
    ...
    out = auth_mod.do_token_refresh({'owa_piggy_profile': 'work'}, audience='graph')
    assert out == 'AT'
    assert captured == [[
...
def test_do_token_refresh_no_profile_when_none(monkeypatch):
    captured = []
    ...
    _patch_owa_piggy(monkeypatch, fake_run)
    assert auth_mod.do_token_refresh({}, audience='outlook') == 'AT'
    assert captured == [['owa-piggy', 'token', '--audience', 'outlook', '--json']]


def test_do_token_refresh_old_json_broker_blocks(monkeypatch, capsys):
    def fake_run(argv, *args, **kwargs):
        if argv == ['owa-piggy', '--version']:
...
def test_do_token_refresh_unparseable_version_does_not_block(monkeypatch):
    def fake_run(argv, *args, **kwargs):
        if argv == ['owa-piggy', '--version']:
    ...
    _patch_owa_piggy(monkeypatch, fake_run)
    assert auth_mod.do_token_refresh({}) == 'AT'


def test_do_token_refresh_oserror_on_subprocess(monkeypatch, capsys):
    def fake_run(argv, *args, **kwargs):
        if argv == ['owa-piggy', '--version']:
            return _version_ok()
...
def test_do_token_refresh_nonzero_rc_passes_stderr(monkeypatch, capsys):
    def fake_run(argv, *args, **kwargs):
        if argv == ['owa-piggy', '--version']:
            return _version_ok()
...
def test_do_token_refresh_non_json_output(monkeypatch, capsys):
    def fake_run(argv, *args, **kwargs):
        if argv == ['owa-piggy', '--version']:
            return _version_ok()
...
def test_do_token_refresh_missing_access_token(monkeypatch, capsys):
    def fake_run(argv, *args, **kwargs):
        if argv == ['owa-piggy', '--version']:
            return _version_ok()
