...
owa-people does not own any auth credentials. It shells out to the
`owa-piggy` CLI (must live in $PATH) and consumes its `--json` token
output on the Graph audience.

The OWA SPA client owa-piggy borrows carries People.Read,
Contacts.Read, and User.ReadBasic.All on the Graph audience, which
is what every endpoint owa-people calls actually needs.
"""
import json
import shutil
import subprocess
...
def _check_owa_piggy_version():
    ...
    if found < MIN_OWA_PIGGY_VERSION:
        ...
        print(
            f'ERROR: owa-piggy {have} is too old; owa-people needs >= {floor}. '
...
def _refresh_via_owa_piggy(config, debug=False):
    """Shell out to `owa-piggy token --audience graph --json [--profile <alias>]`."""
...
def setup_auth(config, debug=False):
    ...
    if not access:
        ...
        suffix = (
            ' or adjust the profile with `owa-people config --profile <alias>`.'
            if profile else '.'
        )
        print(
