"""Token acquisition. Same shape as owa-people/owa-sched: shell out
to owa-piggy on PATH, audience=graph. The OWA SPA scopes carry
...
def _check_owa_piggy_version():
    ...
    if found < MIN_OWA_PIGGY_VERSION:
        ...
        print(
            f'ERROR: owa-piggy {have} is too old; owa-drive needs >= {floor}. '
...
def setup_auth(config, debug=False):
    ...
    if not access:
        ...
        suffix = (
            ' or adjust the profile with `owa-drive config --profile <alias>`.'
            if profile else '.'
        )
        print(
