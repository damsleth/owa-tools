...
```bash
# Manifest-URL path
owa-vids get --manifest-url 'https://globex-mediap.svc.ms/transform/videomanifest?docid=...&format=dash' --profile globex -o meeting.mp4

# Embed-URL path (region must already be cached or --region passed)
owa-vids get --embed-url 'https://tenant-my.sharepoint.com/personal/user/_layouts/15/embed.aspx?uniqueId=...' --profile globex
```
...
```bash
owa-vids check --manifest-url 'https://globex-mediap.svc.ms/transform/videomanifest?docid=...&format=dash' --profile globex
```
