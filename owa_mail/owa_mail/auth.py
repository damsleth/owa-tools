...
owa-mail does not own any auth credentials. It shells out to the
...
owa-mail stores nothing more than an optional `owa_piggy_profile`
...
(`outlook.office.com`), matching `owa-cal` / `owa-graph` and the OWA
SPA client owa-piggy borrows.
"""
import json
import shutil
import subprocess
...
def setup_auth(config, debug=False):
    ...
    if not access:
        ...
        suffix = (
            f' or adjust the profile with `owa-mail config --profile <alias>`.'
            if profile else '.'
        )
        print(
