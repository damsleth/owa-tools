...
### Phase 0 — Foundation _(Agent F0, sonnet)_ — ✅ DONE 2026-06-16 — ships independently, gates everything
_Landed: 4 audiences (`ic3`/`csa`/`presence`/`uis`) added to `AUDIENCE_API_BASE` (now key-set-equal to owa-piggy 0.16.2's 17 `KNOWN_AUDIENCES`); new `AUDIENCE_DESC`; `src/owa_graph/data/audience_seeds.json` (≥1 seed per audience — Tier C Teams paths + Tier D placeholders are **best-effort, unverified against a live token**); `owa_core.jwt.tenant_id`; completions (note: `src/completions/`) + `print_help()` + `docs/graph.md` expanded 13→17; gate test `src/tests/graph/test_audience_table.py` (literal 17-name snapshot). Full suite green, coverage 90.23%._
...
### Phase 1 — Parallel core build _(two independent OPUS agents, wholly disjoint files)_ — ✅ DONE 2026-06-16
_Landed: **A1** `src/owa_graph/tui_nav.py` (`FetchResult`/`Row`, curses-safe `_tui_get` with the auth/scope/notfound/ratelimit/error mapping, `classify_response` incl. the non-JSON→`opaque` branch, `build_rows` with `MAX_ROWS`/`MAX_KEYS` caps + sentinels + opaque/scalar handling, 3-shape `next_path`, `_fetch_page` with OData/ARM/devops continuation incl. case-insensitive header lookup, the graph prefix-index + template normalization + `scope_hint`) + `src/tests/graph/conftest.py` fixtures + `src/tests/graph/test_tui_nav.py` (32 tests). **A2** `src/owa_graph/tui_settings.py` (7 keys, on/off toggles, bookmarks JSON helpers), `config.py` ALLOWED_KEYS (+7), and `src/owa_graph/tui.py` shipping the **final** `TokenInfo` + `GraphState` (a `tui_kit.BrowserState` subclass — loop-owned fields come from the kit, graph session fields attached; this reconciles the pre-kit `_State` with the frozen kit contract) + `_ensure_token` (exp-aware cache, evict-on-failure, `expires_at=None` TTL coercion, `scopes` from `scopes_in_token`) + `test_tui_auth.py`/`test_tui_settings.py`. Gates green; owa_core 96%, combined 90.45%, ruff clean._

**Note for Phase 2 (C1):** `_ensure_token` sets `state.status='minting token for <audience>…'` before the blocking mint, but the kit's loop draws *after* `fetch_items` returns — so C1 must arrange a pre-mint redraw (e.g. a redraw hook passed into the fetch callback) for the "minting…" frame to actually paint before the subprocess blocks. `_fetch_page` returns `(kind, payload, next_cursor)` where `kind` is a status-kind on failure (auth/scope/…) or a classify-kind on success (collection/object/scalar/opaque) — the two sets are disjoint, so C1 switches on it directly.
...
### Phase 2 — Curses front-end _(Agent C1, sonnet; R1 scrutinizes the divergent bits)_ — fan-in on Phase 1 — ✅ DONE 2026-06-17
_Landed: `tui.py` extended with the `tui_kit.app.BrowserSpec` callbacks (`render_row`, state-bound `render_detail` with `format_pretty` gated STRICTLY to `audience=='graph'`, `fetch_items` curses-safe mint+fetch via `_ensure_token`/`_fetch_page` switching on the disjoint kind-union, `on_drill`/`on_back` 7-field history, `on_search` jump keeping prior items, `on_refresh`, actions a/n/e/c/y/o/m/D) + `build_spec(state)` + `run()` lifecycle (stderr redirect owned by `run()` with try/finally; first mint+fetch inside the first loop iteration; graceful seed-failure degradation). `tui_menu.py` (`GraphMenu` on `tui_kit.menu`, 7 settings fields incl. bookmarks). Tests: `test_tui_loop.py` + `test_tui_actions.py` (≈58 graph tests). owa-graph is the **first real consumer of `tui_kit.app`'s `BrowserSpec` loop**._

**Adversarial review (sonnet R1-style) + orchestrator fixes applied:** H1 — `_action_yank_url` `subprocess.run` was missing `capture_output=True`, so an `xclip`/`xsel` diagnostic could hit inherited fd 2 and corrupt the frame (a real curses-safe-invariant breach on Linux/headless) → fixed. M1 — `webbrowser.open` could leak a launcher diagnostic to fd 2 → fixed by a shared `tui_kit.screen.silence_os_fds()` context manager (NEW kit helper, additive; lifted out of an inlined graph class so owa-cal reuses it). M2 — bookmarks added to the settings menu. M3 — action/menu/run coverage raised (`tui.py` 64→84%, `tui_menu.py` 0→90%). L1/L2/L3/L5 cleanups (module-level `_FAILURE_KINDS`, dropped inline imports, `build_spec(state)` removes the `NotImplementedError` placeholder).

**Deviations carried to Phase 3 (not blockers):** (1) the "minting token for <aud>…" status is set but the kit draws *after* `fetch_items` returns, so it isn't painted before the blocking mint — needs a pre-fetch redraw hook. (2) `a` is an interim round-robin audience cycle, not a selectable overlay (the kit's `actions[key](state)` contract passes no `stdscr`, so an in-action overlay/prompt can't be drawn — **see the kit-enhancement note in [owa-suite-tui-rollout.md]**). The load-bearing invariant (commit audience even on a failed switch so `r` retries) holds.
